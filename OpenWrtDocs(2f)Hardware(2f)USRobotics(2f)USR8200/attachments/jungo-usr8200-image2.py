...
#
# A script that can communicate with jungo-based routers to
# backup the installed firmware and replace the boot loader.
#
# Tested with Python 2.5 on Linux and Windows
# Adapted to work on a USR8200 by JohnWoods
...
def image_dump(tn, dumpfile):
    if not dumpfile:
        ...
        if i < 0:
            ...
            sys.exit(1)
        dumpfile = "mi424wr-%s.bin" % buf[i+4:i+21].replace(':','')
    else:
        tn.write("\n")

    dumpsize=8*1024*1024
    ...
    if buf:
        i = buf.find('mtd0:')
        if i > 0:
            dumpsize = int(buf[i+6:].split()[0],16)

    print "Dumping flash contents (%dMB) to %s\n" % (dumpsize/1048576, dumpfile)
    f = open(dumpfile, "wb")

    for addr in range(dumpsize/dumplen):
	if verbose:
	    sys.stdout.write('.')
	    sys.stdout.flush()
