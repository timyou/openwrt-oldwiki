...
{
	float secs = do_run_memory_bench();
...
float run_compute_pi()
{
	...
	secs = ((end-begin)/1000000.0);
	printf("Time to run computation of pi (2400 digits, 100 times): %.2f[secs]\n", secs);
...
int main()
{
	...
	struct tm ti;
       	localtime_r(&t, &ti);
	printf("You can copy/paste the following line in the wiki table at: http://wiki.openwrt.org/HardwarePerformance\n");

	printf("|| %02d-%02d-%04d || ''Author'' || %.1fs || %.1fs || %.1fs || " VERSION_STR " || ''DeviceModel'' || ''CPU model'' || ''CPU Frequency'' || ''LinkToHwPage'' ||\n", ti.tm_mday, (ti.tm_mon+1), (ti.tm_year+1900), sec_mem, sec_pi, sec_e);
